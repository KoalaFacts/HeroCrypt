...
public static class CryptographicWalletExample
{
    public static async Task RunAsync()
    {
        ...
        Console.WriteLine($"  Root Key (Base58/Hex): {Convert.ToBase64String(walletRoot.PrivateKey)[..20]}...");
        Console.WriteLine();
        ...
        var btcKey = HeroCryptBuilder.HdWallet()
            .WithMnemonic(mnemonic)
            .WithPassphrase(passphrase)
            .WithPath(btcPath)
            .Derive();

        Console.WriteLine($"  BTC Private Key: {Convert.ToBase64String(btcKey.PrivateKey)[..40]}...");
        Console.WriteLine($"  BTC Public Key:  {Convert.ToBase64String(btcKey.PublicKey)[..40]}...");
        Console.WriteLine();
        ...
        var ethKey = HeroCryptBuilder.HdWallet()
            .WithMnemonic(mnemonic)
            .WithPassphrase(passphrase)
            .WithPath(ethPath)
            .Derive();

        Console.WriteLine($"  ETH Private Key: {Convert.ToBase64String(ethKey.PrivateKey)[..40]}...");
        Console.WriteLine($"  ETH Public Key:  {Convert.ToBase64String(ethKey.PublicKey)[..40]}...");
        Console.WriteLine();
