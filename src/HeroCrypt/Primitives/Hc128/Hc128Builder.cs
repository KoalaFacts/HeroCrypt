...
namespace HeroCrypt.Primitives.Hc128;

/// <summary>
/// Fluent builder for HC-128 stream cipher operations.
...
/// using var builder = Hc128Builder.Create()
///     .WithKey(key)
///     .WithIv(iv);
...
/// </code>
/// </example>
public sealed class Hc128Builder : IDisposable
{
    ...
    public static Hc128Builder Create() => new();
    ...
    public Hc128Builder WithKey(byte[] key)
    {
    ...
    /// <returns>The builder instance for method chaining.</returns>
    /// <exception cref="ArgumentException">If key is not 16 bytes.</exception>
    public Hc128Builder WithKey(ReadOnlySpan<byte> key)
    {
        if (key.Length != KeySize)
        {
            throw new ArgumentException($"Key must be {KeySize} bytes, but was {key.Length} bytes.", nameof(key));
    ...
    /// <exception cref="ArgumentException">If iv is not 16 bytes.</exception>
    public Hc128Builder WithIv(byte[] iv)
    {
    ...
    /// <returns>The builder instance for method chaining.</returns>
    /// <exception cref="ArgumentException">If iv is not 16 bytes.</exception>
    public Hc128Builder WithIv(ReadOnlySpan<byte> iv)
    {
        if (iv.Length != IvSize)
        {
            throw new ArgumentException($"IV must be {IvSize} bytes, but was {iv.Length} bytes.", nameof(iv));
    ...
    /// </summary>
    /// <returns>The builder instance for method chaining.</returns>
    public Hc128Builder WithRandomIv()
    {
        ClearIv();
        iv = new byte[IvSize];
    ...
    /// <summary>
    /// Transforms the input data using the HC-128 stream cipher.
