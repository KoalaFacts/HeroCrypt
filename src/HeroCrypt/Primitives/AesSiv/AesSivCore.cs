...
/// <summary>
/// Result of an AES-SIV encryption operation.
/// </summary>
public readonly struct AesSivEncryptionResult
{
    /// <summary>The encrypted ciphertext including the 16-byte SIV (synthetic IV) at the beginning.</summary>
    ...
    /// <summary>Size of the SIV (Synthetic IV) in bytes.</summary>
    public readonly int SivSize { get; init; }

    /// <summary>Length of the ciphertext portion (excluding SIV).</summary>
    public readonly int CiphertextLength => Ciphertext.Length - SivSize;
}
...
internal static class AesSivCore
{
    ...
    /// <param name="nonce">Nonce (can be empty for deterministic encryption). Default: auto-generated 16-byte random nonce.</param>
        ...
        bool deterministicMode = false)
    {
        ...
        if (nonce.IsEmpty)
        {
            ...
            {
                // RFC 5297 deterministic mode: no nonce
                nonceArray = [];
            }
            else
            {
        ...
        var ciphertext = new byte[SIV_SIZE + plaintext.Length];
        EncryptCore(ciphertext, plaintext, key, nonceArray, associatedData);

        return new AesSivEncryptionResult
        {
    ...
    /// </summary>
    /// <param name="ciphertext">Input ciphertext including SIV.</param>
    /// <param name="key">AES-SIV key (32, 48, or 64 bytes for AES-128/192/256).</param>
    /// <param name="nonce">Nonce used during encryption. Use empty for deterministic mode.</param>
        ...
        ReadOnlySpan<byte> associatedData = default)
    {
        ...
        {
            throw new ArgumentException($"Ciphertext must be at least {SIV_SIZE} bytes, but was {ciphertext.Length} bytes", nameof(ciphertext));
        }
        ...
        var result = DecryptCore(plaintext, ciphertext, key, nonce, associatedData);

        if (result < 0)
        {
            SecureMemoryOperations.SecureClear(plaintext);
            throw new CryptographicException("AES-SIV decryption failed: authentication tag mismatch.");
        }
    ...
    private static void ValidateKeySize(ReadOnlySpan<byte> key)
    {
        ...
        throw new ArgumentException($"Key must be 32, 48, or 64 bytes (AES-SIV-128/192/256), but was {key.Length} bytes", nameof(key));
    }
}
