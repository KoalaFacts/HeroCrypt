...
/// <summary>
/// Result of an AES-OCB encryption operation.
/// </summary>
public readonly struct AesOcbEncryptionResult
{
...
internal static class AesOcbCore
{
    ...
    /// <param name="nonce">Nonce (1-15 bytes, 12 recommended). Default: auto-generated 12-byte random nonce.</param>
        ...
        bool deterministicMode = false)
    {
        ...
        return new AesOcbEncryptionResult
        {
    ...
    /// <param name="key">AES key (16, 24, or 32 bytes for AES-128/192/256).</param>
    /// <param name="nonce">Nonce used during encryption (1-15 bytes).</param>
        ...
        ReadOnlySpan<byte> associatedData = default)
    {
        ...
        if (result < 0)
        {
            ...
            throw new CryptographicException("AES-OCB decryption failed: authentication tag mismatch.");
        }
    ...
    private static void ValidateParameters(ReadOnlySpan<byte> key, ReadOnlySpan<byte> nonce, int ciphertextLength, int plaintextLength)
    {
        ...
        {
            throw new ArgumentException($"Key must be 16, 24, or 32 bytes, but was {key.Length} bytes", nameof(key));
        }
        if (nonce.Length is < MIN_NONCE_SIZE or > MAX_NONCE_SIZE)
        {
            throw new ArgumentException($"Nonce must be between {MIN_NONCE_SIZE} and {MAX_NONCE_SIZE} bytes, but was {nonce.Length} bytes", nameof(nonce));
        }
