...
internal static class AesCmacCore
{
    ...
    public static void ComputeTag(Span<byte> tag, ReadOnlySpan<byte> data, ReadOnlySpan<byte> key)
    {
        ...
        {
            throw new ArgumentException($"Tag must be at least {BLOCK_SIZE} bytes", nameof(tag));
        }
        ...
        try
        {
            ...
            Span<byte> k1 = stackalloc byte[BLOCK_SIZE];
            Span<byte> k2 = stackalloc byte[BLOCK_SIZE];
    ...
    private static void GenerateSubkeys(Span<byte> k1, Span<byte> k2, ICryptoTransform aes)
    {
        ...
        Span<byte> l = stackalloc byte[BLOCK_SIZE];
        var zeroArray = new byte[BLOCK_SIZE];  // All zeros by default
        var lArray = new byte[BLOCK_SIZE];

        aes.TransformBlock(zeroArray, 0, BLOCK_SIZE, lArray, 0);
        lArray.CopyTo(l);
        ...
        {
            k1[BLOCK_SIZE - 1] ^= 0x87; // Rb constant for 128-bit block
        }
        ...
        {
            k2[BLOCK_SIZE - 1] ^= 0x87; // Rb constant for 128-bit block
        }
    ...
    {
        var n = (data.Length + BLOCK_SIZE - 1) / BLOCK_SIZE; // Number of blocks (ceiling division)
        ...
        var lastBlockComplete = (data.Length > 0) && (data.Length % BLOCK_SIZE == 0);
    ...
    public static bool VerifyTag(ReadOnlySpan<byte> tag, ReadOnlySpan<byte> data, ReadOnlySpan<byte> key)
    {
        if (tag.Length != BLOCK_SIZE)
        {
            throw new ArgumentException($"Tag must be {BLOCK_SIZE} bytes", nameof(tag));
        }

        Span<byte> computedTag = stackalloc byte[BLOCK_SIZE];
