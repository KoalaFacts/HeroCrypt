...
namespace HeroCrypt.Primitives.XSalsa20;

/// <summary>
/// Fluent builder for XSalsa20 stream cipher operations.
...
/// using var builder = XSalsa20Builder.Create()
///     .WithKey(key)
///     .WithNonce(nonce);
...
/// </code>
/// </example>
public sealed class XSalsa20Builder : IDisposable
{
    ...
    public static XSalsa20Builder Create() => new();
    ...
    public XSalsa20Builder WithKey(byte[] key)
    {
    ...
    /// <returns>The builder instance for method chaining.</returns>
    /// <exception cref="ArgumentException">If key is not 32 bytes.</exception>
    public XSalsa20Builder WithKey(ReadOnlySpan<byte> key)
    {
        if (key.Length != KeySize)
        {
            throw new ArgumentException($"Key must be {KeySize} bytes, but was {key.Length} bytes.", nameof(key));
    ...
    /// <exception cref="ArgumentException">If nonce is not 24 bytes.</exception>
    public XSalsa20Builder WithNonce(byte[] nonce)
    {
    ...
    /// <returns>The builder instance for method chaining.</returns>
    /// <exception cref="ArgumentException">If nonce is not 24 bytes.</exception>
    public XSalsa20Builder WithNonce(ReadOnlySpan<byte> nonce)
    {
        if (nonce.Length != NonceSize)
        {
            throw new ArgumentException($"Nonce must be {NonceSize} bytes, but was {nonce.Length} bytes.", nameof(nonce));
    ...
    /// </summary>
    /// <returns>The builder instance for method chaining.</returns>
    public XSalsa20Builder WithRandomNonce()
    {
        ClearNonce();
    ...
    /// <summary>
    /// Sets the initial counter value for the XSalsa20 stream.
    /// </summary>
    /// <param name="counter">The initial counter value (default is 0).</param>
    /// <returns>The builder instance for method chaining.</returns>
    public XSalsa20Builder WithCounter(uint counter)
    {
    ...
    /// <summary>
    /// Transforms the input data using the XSalsa20 stream cipher.
